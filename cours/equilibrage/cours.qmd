...
Passage à la partie sur le tirage équilibré
...
Un plan est **équilibré** sur $\mathbf{x}$ si :
...
Tirer un échantillon de taille fixe et équilibré en transformant progressivement le vecteur des probabilités d'inclusion d'ordre 1 $\pi = (\pi_1, \ldots, \pi_N)$ vers un vecteur d'inclusion $I = (I_1, \ldots, I_N)$.
...
- $\displaystyle \sum_{k \in U} \frac{\textbf{x}_k}{\pi_k} I_k  = \sum_{k \in U} x_k$ (respect de l'équilibrage).
:::
...
f2 <- plot_cube_plan_vecteurs(p1, list(q1, q2, q3), plane_normal,
 affichage_fleche = FALSE, affichage_hyperplan = TRUE)
f2
```
:::
:::
...
-  Ici : 
    ...
    - $A = (1, 1, 1)$
    - $\color{red}{(-0.57, 0.78, -0.21) \in \text{Ker}(A)}$
...
- Si par ailleurs, on suppose la normalité de $(\hat{t}_{y,\text{HT}}, \hat{t}_{\textbf{x},\text{HT}}^T)^T$ : $$\mathbb{V}_{p_\text{Poisson}}(\hat{t}_{y,\text{HT}} | \hat{t}_{\textbf{x},\text{HT}} = t_\textbf{x}) \approx \mathbb{V}_{p_\text{Poisson}}(\hat{t}_{y,\text{HT}} - (t_x - \hat{t}_{\textbf{x},\text{HT}})^T \beta)$$
...
Plusieurs choix de $\beta$ sont possibles, par exemple :
